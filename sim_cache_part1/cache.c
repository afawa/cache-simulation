	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  if (!blk_access_fn){
    printf("must specify miss/replacement functions");
    exit(1);
  }
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if ((nbytes & (nbytes-1)) != 0 || (addr & (nbytes-1)) != 0){
    printf("cache: access error: bad size or alignment, addr 0x%08llx", addr);
    exit(1);
  }
  ...
  if ((addr + nbytes) > ((addr & ~cp->blk_mask) + cp->bsize)){
    printf("cache: access error: access spans block, addr 0x%08llx", addr);
    exit(1);
  }
...
/*需要实现 blk_access_fn
 *这是当cache miss或replacement时调用的,miss对应read,replacement对应write
 *用于和下一级进行连接
 * */
